...
EXCLUDED_NETWORKS = ['91.108.13.0/24', '149.154.160.0/21', '149.154.160.0/22',
                     ...
                     '91.108.58.0/23', '91.108.8.0/22', '95.161.64.0/20', '35.215.130.0/24',
                     '13.250.0.0/15', '35.215.0.0/18', '35.213.0.0/18', '54.255.0.0/17', 
                     '95.183.128.0/17',
                     '35.215.0.0/24','35.215.183.0/24', '35.215.129.0/24', '35.215.131.0/24', '35.215.149.0/24', '157.240.235.0/24', '157.240.15.0/24', '157.240.13.0/24', '157.240.0.0/24', '157.240.235.0/24']
...
def is_local_ip(ip):
    """Check if the IP address is local or private."""
    ...
    for packet in cap.sniff_continuously(packet_count=999999):
        if hasattr(packet, 'ip'):
            ...
            if packet.stun:
                ...
                # if xor_mapped_user:
                #     print(f"[+] XOR-MAPPED-ADDRESS has user: {xor_mapped_user}")
...
def main():
    try:
        ...
        result = extract_stun_xor_mapped_address(interface_name)
        if result:
            xor_mapped_address, xor_mapped_user = result  # Unpack the result tuple
            print(f"[+] SUCCESS! IP Address: {xor_mapped_address}, User: {xor_mapped_user}")
            whois_data = get_whois_info(xor_mapped_address)  # Pass only the IP address
